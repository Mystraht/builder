...
class UIBuilder
{
			...
			for (lMovie in lData.movies) {
				if (lMovie.id == pId) {
					for (lItem in lMovie.layers) {
						//trace("Item name " + lItem.keyframes[0].ref);
						...
						var lPosition:String = uiPos.get(lItem.name.split("_")[0]);
						var lFit:String = uiPos.get(lItem.name.split("_")[1]);
						lUIPos.push(getUIPositionable(lObj, lPosition, lFit));
					}
				}
			}
		}
	...
	private static function getTextFromJson(pName:String) : Text
	{
		...
		var text:Text = new Text(localizedText, style);
		if (text.height <= uiStyle.size * 2 && text.style.align == "center") {
	 ...
	 * @param 	pFit scale de l'Obj
	 * @return un UIPositionable
	 */
	private static function getUIPositionable (pObj:Container,pPosition:String, ?pFit:String = null):UIPositionable {
		...
		if (pFit != UIPosition.FIT_HEIGHT &&
			pFit != UIPosition.FIT_SCREEN &&
			pFit != UIPosition.FIT_WIDTH) pFit = "";
			
		if (pPosition == UIPosition.FIT_HEIGHT
			|| pPosition == UIPosition.FIT_WIDTH
			|| pPosition == UIPosition.FIT_SCREEN) pFit = pPosition;
