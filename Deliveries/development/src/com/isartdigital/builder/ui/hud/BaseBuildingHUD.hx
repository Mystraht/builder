...
class BaseBuildingHUD extends UIComponent
{
	...
	public var hardBuildButton:HardBuildButton;
	...
	private static inline var BUTTON_UPGRADABLE_NAME:String = "UpgradeButton";
	...
	private var currentBuildingEventDef:BuildingEventDef;
	...
	public function new() 
	{
		...
		upgradeButton = cast(getChildByName(BUTTON_UPGRADABLE_NAME), UpgradeButton);
		...
		upgradeButton.click = upgradeButton.tap = onUpgradeClick;
	...
	private function onUpgradeClick (event:EventTarget) : Void {
		var config:Dynamic = currentBuildingEventDef.ref.getConfig();
		var nextLevelConfig:Dynamic = BuildingUtils.getConfigByName(
			currentBuildingEventDef.ref.definition.name,
			currentBuildingEventDef.ref.buildingLevel + 1
		);
		var params:UpgradeConfirmParamsDef = { 
			timeLeft : nextLevelConfig.contruction_time,
			price :config.upgrade_price
		};
		UIManager.getInstance().emit(UIManager.OPEN_POPIN_REQUEST_UPGRADECONFIRM, params);
	}
