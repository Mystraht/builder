...
class ShopCategory extends UIComponent
{
	...
	public function getLeftItemBuilding () : ItemShop {
		return itemList[0];
	}
	
	public function getRightItem () : ItemShop {
	...
	private function appearArrows () : Void {
		if (Math.floor(positionInShop / buildingPerSheet) ==  dotList.length-1) hideObject(rightArrowList[0]);
		else appearFeedBackObject(rightArrowList[0]);
		
		if (Math.floor(positionInShop / buildingPerSheet) == 0) hideObject(leftArrowList[0]);
		else appearFeedBackObject(leftArrowList[0]);
	}
	
	private function hideObject (object:DisplayObject) : Void {
		object.visible = false;
	}
	
	private function appearFeedBackObject (object:DisplayObject) : Void {
		object.visible = true;
		object.scale.set(1.5, 1);
		Actuate.tween(object.scale, 0.5, { x : 1 } ).ease(Bounce.easeOut);
	}
