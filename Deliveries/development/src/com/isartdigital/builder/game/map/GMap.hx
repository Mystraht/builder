...
class GMap
{
	...
	/**
	 * @deprecated don't use it for performance reason
	 *
	...
	public static function getRandomAvailablePosition():Point {
		var availableTiles:Array<TileModelDef> = Tile.getBuildableTiles();
		var availableTile:TileModelDef = availableTiles[Math.floor(Math.random() * availableTiles.length)];
		return new Point(availableTile.x, availableTile.y);
	}
