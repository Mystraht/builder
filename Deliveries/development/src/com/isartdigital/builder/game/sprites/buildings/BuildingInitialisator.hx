...
class BuildingInitialisator {
	...
	public function initialisate(buildingModelDef:BuildingModelDef):Void {
		...
		removeLevelIfBuildingIsUpgrading();
	...
	private function removeLevelIfBuildingIsUpgrading():Void {
		if (building.buildingTimebase.isUpgradingState()) {
	...
	private function createZSortableConfiguration():Void {
		...
		building.modelWidth = building.definition.size.width;
		building.modelHeight = building.definition.size.height;
	}
