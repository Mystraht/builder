...
class LanternStrategy extends Building {
	...
	public function lanternInteraction() {
		if (BuildingUtils.thereIsOtherBuildingMovingThan(lantern) || UIManager.mouseIsOverUI) {
			return;
		}
		
		if (isLanternAlreadyIlluminated()) {
			UIManager.getInstance().emit(UIManager.OPEN_POPIN_REQUEST_LANTERNINFO);
		} else {
			UIManager.getInstance().emit(UIManager.OPEN_POPIN_REQUEST_LANTERNCONFIRM);
			listenBuyEvent();
			Ftue.event.emit(FtueEvents.LANTERN_SELECT);
		}
	}
	...
	private function buyWith(currency:String) {
		...
		if (thereIsEnoughtResources()) {
			...
			if (!Ads.getImage(onAdImageEnd)) return;
		} else {
	...
	private function onAdImageEnd (params:Dynamic) : Void {
		return;
	}
	...
	private function setHardAmountToSpend():Void {
		...
		ressourceAmountToSpend = lanternConfig.hard_price * Users.getTotalIlluminatedLanterns() + lanternConfig.base_hard_price;
	}

	private function setNormalAmountToSpend():Void {
		var lanternConfig = lantern.getConfig();
		ressourceTypeToSpend = RessourceManager.GOLD;
		ressourceAmountToSpend = lanternConfig.price * Users.getTotalIlluminatedLanterns() + lanternConfig.base_price;
	}
