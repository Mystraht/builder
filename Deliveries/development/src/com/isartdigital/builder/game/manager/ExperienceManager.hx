	...
	public function updateLevelAndXp(?params:Dynamic):Void {
		...
		for (i in 0...levelsArray.length) {
			if (lXpTotalUsers < levelsArray[i]) {
				if (lvlUser < i) {
					UIManager.getInstance().emit(UIManager.OPEN_POPIN_REQUEST_LEVELREWARD);
					Facebook.shareOnWall({message: 'Je suis monté niveau ' + i + ' sur Fiesta Ultima ! :D', link: Facebook.FACEBOOK_APP_LINK} );
				}
				lvlUser = i;
