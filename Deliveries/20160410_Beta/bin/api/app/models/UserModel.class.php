...
class UserModel extends ApplicationModel
{
	...
	public function validate($params, $type) {
		...
		if ($type == 'paradeUpdate') {
			$errorMessage .= $this->isPresent($params, 'token', "Le token doit être présent\n");
			$errorMessage .= $this->isPresent($params, 'bonusPesos', "Le nombre de bonusPesos est absent\n");
			$errorMessage .= $this->isPresent($params, 'bonusPimientos', "Le nombre de bonusPimientos est absent\n");
			$errorMessage .= $this->isPresent($params, 'bonusOffering', "Le nombre de bonusOffering est absent\n");
			$errorMessage .= $this->isPresent($params, 'useHardInParade', "Le champ useHardInParade est absent\n");
	...
	public static function getUserData($userId) {
		$error = '';
		$data;
		$data['experience'] = UsersController::getExperience($userId);
		
		$currencyJson = ResourcesController::getResource($userId);
		$currency = json_decode($currencyJson, true);
		
		$data = array_merge($currency['data'], $data);
