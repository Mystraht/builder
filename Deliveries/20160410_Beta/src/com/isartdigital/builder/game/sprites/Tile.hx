...
class Tile extends SpriteObject
{
	...
	private static var buildableTiles:Array<TileModelDef> = new Array<TileModelDef>();
	...
	public static function illumineTileInRadiusAt(position:Point, radius:Int):Void {
		...
		for (k in 0...tiles.length) {
			if (GMap.isInsideGrid(Std.int(tiles[k].x), Std.int(tiles[k].y))) {
				var illuminationPosition = new Point(tiles[k].x, tiles[k].y);
				BuildingUtils.setBuildingVisibleAt(illuminationPosition);
	...
	private static function setTileIlluminatedAndAlphaInGlobalMapAt(tile:TileModelDef):Void {
		...
		if (!tileInGlobalMap.isIlluminated) {
			...
			cacheTileInBuildableTilesArray(tileInGlobalMap);
		}
	}
	 ...
	 * @param	isBuildable
	 */
	public static function changeTilesBuildableState(tiles:Array<TileModelDef>, _isBuildable:Bool):Void {
		...
		for (i in 0...tiles.length) {
			...
			if (_isBuildable) {
				cacheTileInBuildableTilesArray(tile);
			} else {
				removeTileFromBuildableTilesArray(tile);
			}
		}
	}
	...
	/**
	 * Récupère une tile à la position donnée
	 */
	public static function getTileAt(position:Point):Tile {
		var tile:Tile;
		...
		throw 'Position ' + position.x + ':' + position.y + ' do not contain Tile';
		
		return null;
	}

	/**
	 * Récupère la liste des tiles constructible
	 */
	public static function getBuildableTiles():Array<TileModelDef> {
	...
	private static function cacheTileInBuildableTilesArray(tile:TileModelDef):Void {
		if (buildableTiles.indexOf(tile) == -1) {
			buildableTiles.push(tile);
		}
	}

	private static function removeTileFromBuildableTilesArray(tile:TileModelDef):Void {
		if (buildableTiles.indexOf(tile) != -1) {
			buildableTiles.splice(buildableTiles.indexOf(tile), 1);
		}
	}
}