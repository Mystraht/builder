...
class Building extends SpriteObject implements IZSortable
{
	...
	public function select():Void {
		...
		selectBuildingAndSubscribeHudEvent();
	}
	...
	private function selectBuildingAndSubscribeHudEvent():Void {
		setBuildingSelected();
		subscribeHudEvent();
	}
	...
	private function updateHarvestIconState():Void {
		if (isHarvestable() && !isSelected && !isMoving) {
			desactiveHarvestingIfContructingOrUpgradingState();
			buildingHarvester.updateHarvestIconState();
		}
	}

	private function desactiveHarvestingIfContructingOrUpgradingState() {
		if (buildingTimebase.isConstructingOrUpgradingState()) {
	...
	private function applyFilterByConstructibleState():Void {
		...
		if (buildingConstructor.canConstruct()) {
			filters = cast Filter.getBrightness(1.5);
		} else {
	...
	private function onHardBuildRequest(event:Dynamic):Void {
		...
		UIManager.getInstance().emit(UIManager.OPEN_POPIN_REQUEST_HARDBUILDCONFIRM);
		UIManager.getInstance().on(HardBuildConfirm.CONFIRM_BUTTON_CLICK, onHardBuildConfirm);
	}

	private function onHardBuildConfirm(event:Dynamic):Void {
