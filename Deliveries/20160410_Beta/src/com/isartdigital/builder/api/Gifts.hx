...
class Gifts 
{
	...
	public function getGifts(pCallBack:String->Void):Void {
		var request = new Http(ApiUtils.formatPath(Api.domain + Api.pathApi + giftsPath, { token: Api.token }));
	...
	public function create(friendUserId, pCallBack:String->Void):Void {
		var request = new Http(ApiUtils.formatPath(Api.domain + Api.pathApi + giftsPath + createPath, { token: Api.token, friend_user_id: friendUserId}));
	...
	public function collect(name, authorName, pCallBack:String->Void):Void {
		var request = new Http(ApiUtils.formatPath(Api.domain + Api.pathApi + giftsPath + collectPath, { token: Api.token, name: name, author_name: authorName}));
