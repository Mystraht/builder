...
class Lanterns 
{
	...
	public function getLanterns(pCallBack:String->Void):Void {
		var request = new Http(ApiUtils.formatPath(Api.domain + Api.pathApi + lanternsPath, { token: Api.token }));
	...
	public function create(pX: Int, pY:Int, pHardPurchase:Bool, pCallBack:String->Void):Void {
		var request = new Http(ApiUtils.formatPath(Api.domain + Api.pathApi + lanternsPath + createPath, { token: Api.token, x:pX , y:pY, hard: pHardPurchase}));
