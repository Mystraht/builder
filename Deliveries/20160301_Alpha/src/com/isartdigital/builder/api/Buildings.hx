...
class Buildings 
{
	...
	public function create(pBuilding:String, pX:Int, pY:Int, hardBought:Bool, pCallBack:String->Void) {
		var hardBoughtToString:String;
		hardBoughtToString = hardBought ? 'true' : 'false';

		var request = new Http(Utils.formatPath(Api.domain + Api.pathApi + buildingsPath + "/" + pBuilding + createPath , { token: Api.token, x: pX, y: pY, hard: hardBoughtToString }));
