...
extern class Polygon {
	 ...
	 *      all the points of the polygon e.g. `MapManagerTest PIXI.Polygon(MapManagerTest PIXI.Point(), MapManagerTest PIXI.Point(), ...)`, or the
	 *      arguments passed can be flat x,y values e.g. `MapManagerTest PIXI.Polygon(x,y, x,y, x,y, ...)` where `x` and `y` are
	 *      Numbers.
	 */
