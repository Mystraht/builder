...
class UsersController extends ApplicationController
{
	...
	private function createBasemap($userId) {
		...
		$request->execute(array($userId, $date, $date, 14, $GLOBALS['app']['db']->lastInsertId(), 50, 50, $date, 'A'));
		
		$request = $GLOBALS['app']['db']->prepare('INSERT INTO building_main_square VALUES (NULL, ?)');
		$request->execute(array($userId));
		$request = $GLOBALS['app']['db']->prepare('INSERT INTO building_users VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
		$request->execute(array($userId, $date, $date, 8, $GLOBALS['app']['db']->lastInsertId(), 45, 42, $date, 'A'));

		$request = $GLOBALS['app']['db']->prepare('INSERT INTO building_cantina VALUES (NULL, ?, ?, ?)');
		$request->execute(array($userId, 1, $date));
		$request = $GLOBALS['app']['db']->prepare('INSERT INTO building_users VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
		$request->execute(array($userId, $date, $date, 16, $GLOBALS['app']['db']->lastInsertId(), 56, 47, $date, 'A'));
		
		$request = $GLOBALS['app']['db']->prepare('INSERT INTO lantern_users VALUES (NULL, ?, ?, ?, ?, ?)');
		$request->execute(array($userId, $date, $date, 41, 47));

		$request = $GLOBALS['app']['db']->prepare('INSERT INTO lantern_users VALUES (NULL, ?, ?, ?, ?, ?)');
		$request->execute(array($userId, $date, $date, 56, 44));
	}
